...
:wildfly_download: https://www.wildfly.org/downloads/
...
// url=maven_home. text=Apache Maven
The simplest way to get started with Arquillian is to incorporate it into the test suite of a project build that offers dependency management. Today, the most widely used build tool in this category is link:{site.page('maven_home.').url}[Apache Maven]
...
// url=maven_download. text=download and install Maven now
This guide assumes you have Maven available, either in your command shell or your IDE(Integrated Development Environment). If you don't, please link:{site.page('maven_download.').url}[download and install Maven now]
 You'll also need https://adoptium.net/temurin/releases[JDK(Java Development Kit) 1.5] or higher installed on your machine, though JDK 1.6 is preferred.

=== Create a New Project


The main way to create a new Maven project is to use the "archetype" plugin. This plugin provides a set of templates that can be used to create a new project.from a Maven archetype. The details are provided in <<generate_project_from_archetype>>
...
[[generate_project_from_archetype]]
...
_Copy the text after the `$` and paste it into your command shell_. Respond to the prompts by entering the value shown after each double colon in the pane below. Hit the Enter key after each entry (as indicated by `<ENTER>`).
...
[WARNING]
====
Arquillian also supports TestNG 5. However, we'll be using JUnit throughout this guide.
====
...
// url=<<open_the_project_in_eclipse>>, text=Open the Project in Eclipse
p. The foundation of your project is now ready! Skip to the next section, link:{site.page('#open_the_project_in_eclipse,').url}[Open the Project in Eclipse]
...
Begin by launching Eclipse. Since this is a Maven project, you need the http://eclipse.org/m2e[Maven Integration for Eclipse] (m2e) plugin installed in order to open the project.

==== Installing the m2e plugin


// url=tools_home. text=JBoss Tools
If you don't already have the m2e plugin installed, the simplest way to get it is to install link:{site.page('tools_home.').url}[JBoss Tools]
 Follow these steps to install it from the Eclipse Marketplace (sort of like the app store for Eclipse).
...
image::/images/guides/arquillian_tutorial_eclipse_project.png[]
...
// url=https://community.jboss.org/wiki/WhatVersionOfSurefireShouldIUseToRunMyArquillianTestsInAMavenBuild. text=this FAQ
(Optional) We recommend upgrading the Maven Surefire Plugin from the default version, for reasons described in https://community.jboss.org/wiki/WhatVersionOfSurefireShouldIUseToRunMyArquillianTestsInAMavenBuild.[this FAQ] You can set the version of the Surefire Plugin by appending this `<plugin>` element inside the `<plugins>` element, just below the Maven Compiler Plugin:
...
// url=https://raw.github.com/arquillian/arquillian-examples/master/arquillian-tutorial/pom-no-container-profiles.xml text=pom-no-container-profiles.xml
p. If you're having trouble with the pom.xml up to this point, you can download the file https://raw.github.com/arquillian/arquillian-examples/master/arquillian-tutorial/pom-no-container-profiles.xml[pom-no-container-profiles.xml]// url=https://github.com/arquillian/arquillian-examples/tree/master/arquillian-tutorial text=example project
 from the https://github.com/arquillian/arquillian-examples/tree/master/arquillian-tutorial[example project] and rename it to pom.xml.
...
[NOTE]
====
ShrinkWrap also supports resolving artifacts (libraries) and create configuration files programmatically, which can then be added to the test archive. For a more thorough introduction to ShrinkWrap, see the "ShrinkWrap introduction":/guides/shrinkwrap_introduction guide.
====
...
// url=/guides/getting_started_rinse_and_repeat/<<export_the_deployment>>. text=Getting Started: Rinse and Repeat
p. ShrinkWrap assigned the archive a random name since we didn't specify one explicitly. We'll cover another way to inspect the archive in the follow-up guide, link:{site.page('/guides/getting_started_rinse_and_repeat/#export_the_deployment.').url}[Getting Started: Rinse and Repeat]
...
Arquillian selects the target container based on which container adapter is available on the test classpath. A _container adapter_ controls and communicates with a container (e.g., Weld Embedded, WildFly, GlassFish, etc). That means we'll need to add additional libraries to the project.
...
image::/images/guides/arquillian_tutorial_run_junit_test.png[]
...
image::/images/guides/arquillian_tutorial_debugging_test.png[]
...
// url=http://embedded-glassfish.java.net: text=Embedded GlassFish
We'll now include two additional profiles in the `pom.xml` inside the `<profiles>` element, the first for http://embedded-glassfish.java.net:[Embedded GlassFish]
...
and the other for WildFly managed:
...
[NOTE]
====
By default, WildFly deploys test archives to the server using the JMX protocol. We've gone ahead and added the dependency for the Servlet protocol in the @arquillian-jbossas-managed@ profile just so it's there if you need to use it in the future. See "this FAQ":https://community.jboss.org/wiki/WhyDoIGetNoActiveContextsForScopeTypeWhenTestingOnJBossAS7 for instructions on how to switch to it.
====


You now have the choice of running the tests in one of three containers.

// url=https://raw.github.com/arquillian/arquillian-examples/master/arquillian-tutorial/pom.xml text=pom.xml
p. If you're having trouble with the pom.xml up to this point, you can download the file https://raw.github.com/arquillian/arquillian-examples/master/arquillian-tutorial/pom.xml[pom.xml]// url=https://github.com/arquillian/arquillian-examples/tree/master/arquillian-tutorial. text=example project
 from the https://github.com/arquillian/arquillian-examples/tree/master/arquillian-tutorial.[example project]
...
// url=http://www.eclipse.org/m2e): text=Maven Integration for Eclipse
There are two ways to activate a Maven profile in Eclipse (assuming you are using the http://www.eclipse.org/m2e):[Maven Integration for Eclipse]
...
image::/images/guides/arquillian_tutorial_maven_properties.png[]
...
image::/images/guides/arquillian_tutorial_maven_profile_selector.png[]
...
You've now run the same test on two different embedded containers, a CDI container (Weld) and a Java EE container (GlassFish). Both of these executions are in process. To really be sure the component works in a pure environment, we need to use a standalone container. Let's switch to using WildFly.

To run the test on a standalone instance of WildFly, you first need to set it up. You can either:

. Download and unpack it in a location outside the project
. Use Maven to download and unpack it during a build

Follow these steps to setup WildFly outside the project:

. https://www.wildfly.org/downloads/[Download WildFly] (make sure the version you download matches the version of the `jboss-as-arquillian-container-managed` dependency in your `pom.xml`)
. Extract the archive
. (optional) Set the `JBOSS_HOME` environment variable to the path of the extracted directory

If you have WildFly installed (i.e., extracted) and your `JBOSS_HOME` environment variable set to that location, you can activate the `arquillian-jbossas-managed` profile and run the test. You should see WildFly start in the console...and another *green bar*!
...
To target a managed WildFly instance downloaded by Maven, you also need a small bit of Arquillian configuration. Create the following configuration file and assign the value of the `jbossHome` property to the location where WildFly is installed. If you're using the Maven dependency plugin, the location is `target/jboss-as-7.1.1.Final`.
...
Now change the active Maven profile to `arquillian-jbossas-managed`, then run the test again. You should see WildFly starting in the console...and yet another *green bar*!
...
// url=/guides/getting_started_rinse_and_repeat text=Getting Started: Rinse and Repeat

If you want to dive deeper into Arquillian, move on to link:{site.page('guides/getting_started_rinse_and_repeat').url}[Getting Started: Rinse and Repeat]
 guide. To learn how to use Forge to automate Arquillian setup and test generation, read through link:{site.page('guides/get_started_faster_with_forge').url}[Get Started Faster with Forge]
