...
As we are on the way to a first Beta release, we identified a need for sharing some of recent Graphene improvements:

h3. Highlighted Changes

- <i class="icon-star"></i> Guard Improvements and Fixes :=
Requests Guards were polished, extended and hardened. ("read more":#guard-improvements-and-fixes) =:

- <i class="icon-star"></i> Creating Page Fragments Programatically := 
Page Fragments can be now instantiated not only using dependency injection, but also programatically. ("read more":#creating-page-fragments-programatically) =:

- <i class="icon-star"></i> Automatic Inference of Locators := 
Page Fragments can be now instantiated not only using dependency injection, but also programatically. ("read more":#automatic-inference-of-locators) =:
...
- <i class="icon-star"></i> PhantomJS Support and Improved Integration with Drone := 
...
Drone @1.2.0.Alpha2@ has got "@Enhancer@":https://github.com/arquillian/arquillian-extension-drone/blob/master/drone-spi/src/main/java/org/jboss/arquillian/drone/spi/Enhancer.java API which allow us to integrate with Drone seamlessly. =:
...
Not all of us are familiar with abbreviation XHR (XMLHttpRequest). In order to make the API more clear for a most of Graphene users, we have deprecated @guardXhr@ and replaced it with @guardAjax@. @guardXhr@ will be replaced in upcoming releases. =:
...
Guards had problems on Android and with deterministic waiting for HTTP requests. =:
...
p. With Alpha4 we have added possibility to create a page fragment programatically:
...
You can find a reference usage in a "functional test":https://github.com/jhuska/arquillian-graphene/blob/7637bfd0d92fa78b75caa54bbf41bb393868d62a/graphene-webdriver/graphene-webdriver-ftest/src/test/java/org/jboss/arquillian/graphene/enricher/TestEnrichingContainerElement.java#L61.
...
p. Graphene will automatically use strategy "@How.ID_OR_NAME@":http://selenium.googlecode.com/git/docs/api/java/org/openqa/selenium/support/How.html#ID_OR_NAME to locate the element by its ID or name.

Since this mechanism uses strategy pattern, you can overwrite default strategy for your test suite and therefore you can find elements by e.g. their class names or even JSF component IDs.
