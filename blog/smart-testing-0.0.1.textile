...
p. It all started as "Google Summer of Code":https://developer.jboss.org/wiki/JBossCommunityGoogleSummerOfCode2016Ideas#jive_content_id_Provide_test_scheduling_for_large_test_suites project last year with great ground work of our student "Dimcho Karpachev":https://github.com/dimchoSCR. 
...
Moreover having long lasting builds on your CI server makes your CI/CD process not really that continuous and leads to Pull Requests piling up. Bringing new features to production with confidence should be seamless. And for this we need to have faster feedback loops. Smart Testing is a tool that speeds up the test running phase by reordering test execution plan to increase a probability of fail-fast execution and thus give you faster feedback about your project’s health.

That's why we created a cure for you!

h3. Higlights of this release
...
p. With this release we provide following strategies which you can use to optimize your build execution:
  ...
  * @changed@ - gives higher priotity to changed tests (e.g. new test methods or refactorings of components under tests)
  * @affected@ - based on changes in your business logic gives higher priority to tests which are excercising them
...
p. If you are using Maven 3.3.x or newer (and you definitely should!) adding Smart Testing to your build is easy. Create a file `.mvn/extensions.xml` in the root of project 
...
This will execute the build with only those tests which are falling into those categories, based on local changes.
...
This will optimize tests based on the changes between current commit and the one against which the previous build was run.
...
p. We are very excited about Smart Testing but we need you to help us make it even more awesome. We spent tremendous amount of effort building it with user experience in mind, but without the community feedback we can only get that far. So give it a try today and "tell us what you think":http://discuss.arquillian.org/!


p. Thanks to the "whole team":https://github.com/arquillian/smart-testing/graphs/contributors for making it happen! You are real heros!

p. Also big kudos to "Infinitest":http://infinitest.github.io/ team for the inspiration!
