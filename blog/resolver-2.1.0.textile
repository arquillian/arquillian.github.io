...
ShrinkWrap Resolver 2.1.0 brings you support for Maven 3.1.0, additional features and improved configuration API.

h3. Significant additions in ShrinkWrap Resolver 2.1.0

- <i class="icon-star"></i> Support for Maven 3.1.0 and newer :=

This release was updated to sure Eclipse Aether, which makes it compatible with Maven 3.1.0 and newer. ShrinkWrap Resolver Maven Plugin now requires you run it with recent Maven version, other components are compatible with previous Maven version as well. I'd like to thank Tomaž Cerar and Michał Matłoka for making this update possible. =:
...
bc(prettify).. Maven.configureResolver()
    .withRemoteRepo(MavenRemoteRepositories.createRemoteRepository("my-repository-id", "url://to/my/repository", "layout"))
   .resolve("G:A:V").withTransitivity().asFile(); 
