...
* "Bartosz Majsak":https://github.com/bartoszmajsak (Arquillian team member, author of Beer Advisor example)
...
p(info). %The sample tests in this section apply to the "Beer Advisor example":https://github.com/bartoszmajsak/Beer-Advisor.%
...
The Beer Advisor demo uses Thucydides WebDriver factories instead of Drone, and has a "ArquillianEnricher JUnit rule":https://github.com/bartoszmajsak/Beer-Advisor/blob/master/thucydides-tests/src/test/java/org/arquillian/example/thucydides/ArquillianEnricher.java to delegate to the "ArquillianListener":https://github.com/bartoszmajsak/Beer-Advisor/blob/master/thucydides-tests/src/test/java/org/arquillian/example/thucydides/ArquillianListener.java to do the normal test class enrichment/injection. A JUnit Rule is necessary due to missing hooks in Thucydides, but in general it works admirably with Arquillian controlling the Container and Deployment. Vineet has since been able to eliminate the JUnit rule as a result of recent enhancements to Thucydides.
