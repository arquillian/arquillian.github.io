...
p. In @arquillian.xml@ file, properties are always set hardcoded or as system property @${myproperty}@ or environment variable @${env.myenvironment}@. But in both cases values are set before test execution.
...
p. You need to register a new arquillian service in @ExtensionBuilder@ and must conform following signature:
...
p. Currently apart from resolving properties from system properties and environment, we added a resolution of locations from classpath. For example:
