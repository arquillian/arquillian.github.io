---
layout: post
title: Testing with Aliens: How to use Arquillian to test your JPA type converter
author: aslakknutsen, thjanssen
tags: [ testing, persistence, extension ]
---
This post was written together with "Aslak Knutsen":http://plus.google.com/118222277307713726144?rel=author ("@aslakknutsen":https://twitter.com/aslakknutsen).

JPA "type converters":http://www.thoughts-on-java.org/2013/10/jpa-21-how-to-implement-type-converter.html provide a way to define how an entity attribute gets persisted to the database. Writing the type converter is easy, but we also need to make sure, that it is working correctly.
...
We will have a look at how this can be done with "Arquillian":http://www.arquillian.org/ and its database extension.

h3. Something about Arqillian

If you are already familiar with Arquillian, you can skip this part. For all of you who have never worked with Arquillian so far, I just want to give some basic information. You can find a more detailed description at "http://www.arquillian.org"http://www.arquillian.org/.
...
Before we can start to write our tests, we need to define a few dependencies. I will only show how to configure the dependencies we need for this example. If you have not already set up arquillian tests for your project, you will have to do a little bit more. Please check the "Getting Started guide":http://arquillian.org/guides/getting_started/ to learn how to setup arquillian for your project. Don't be afraid, there is not too much to do.

As you can see in the following snippet, we will use junit 4.11, +Arquillian 1.1.3.Final, the Arquillian persistence extension in version 1.0.0.Alpha7 and the wildfly application server in version 8.1.0.Final.
...
We used Arquillian and the Arquillian database extension to test a JPA type converter. Therefore we injected the EntityManager and used the annotations @ShouldMatchData and @UsingDataSet to validate and seed the database with a yml file.
...
I am using it in my daily work and I do not want to miss it. The container management provided by Arquillian and the database seeding and validation feature of the persistence extension makes testing comfortable and efficient.

If you enjoyed reading this article and like to learn more about other Java EE7 or Arquillian features, make sure to subscribe to my blog via "Mail":http://feedburner.google.com/fb/a/mailverify?uri=SomeThoughtsOnJavaee or "RSS feed":http://www.thoughts-on-java.org/feeds/posts/default or follow me on "twitter":https://twitter.com/thjanssen123 and "google+":https://plus.google.com/104438663880735057595.

And follow Aslak Knutsen on "google+":http://plus.google.com/118222277307713726144?rel=author and "twitter":https://twitter.com/aslakknutsen. 
