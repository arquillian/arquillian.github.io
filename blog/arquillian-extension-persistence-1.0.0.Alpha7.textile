...
<i class="icon-star"></i> SQL script handling has been significantly imporoved. It not only has improved performance (namely for Oracle) but also let you provide your own implementation for parsing multi-line scripts through simple SPI @org.jboss.arquillian.persistence.spi.script.StatementSplitter@.

<i class="icon-star"></i> Ability to inject DBUnit @DatabaseConnection@ making it possible to interact with DBUnit directly in the tests.
bc(prettify).. @ArquillianResource
private DatabaseConnection databaseConnection;

<i class="icon-star"></i> Through newly introduce SPI module you can now register your own datasource providers. This can be leveraged by frameworks such as Spring which can now do their own lookups (from application context) instead of relying on JNDI.

<i class="icon-star"></i> We have also implemented extension point to define custom implementation of @org.dbunit.database.DatabaseSequenceFilter@ which is used to seed and clean database by DBUnit. Out of the box we provide sequence filter strategy for Oracle. You can register it in @arquillian.xml@ as part of _persistence-dbunit_
...
p(important). %*_Important change_*: We are still shaping up the extension and its modularity hence we split modules in the following manner:
_persistence-api_
_persistence-core_ 
_persistence-dbunit_ - you can replace old _impl_ depedency by this one
