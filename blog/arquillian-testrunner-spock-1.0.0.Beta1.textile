...
h3(highlights). Important changes
 ...
 <i class="icon-star"></i> In order to let Arquillian sneak in to Spock dimension you need to annotate your @Specification@ using @@ArquillianSpecification@.

 <i class="icon-star"></i> We also updated Spock and Groovy versions, so we guarantee that following combinations won't let you down:
...
You can always take a look at our "examples":https://github.com/bartoszmajsak/arquillian-testrunner-spock/tree/master/examples where we show how to use both versions with Maven.
