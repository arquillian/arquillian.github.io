...
p. This release of Arquillian Cube Q contains one new, but important feature, that allows you to add some randomness on some toxics values, using mathematical distributions. Currently only https://en.wikipedia.org/wiki/Uniform_distribution_(continuous):[uniform distribution] and https://en.wikipedia.org/wiki/Log-normal_distribution:[log-normal distribution] are supported.
