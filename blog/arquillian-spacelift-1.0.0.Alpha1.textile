...
p. For a long time we have struggeled with using tools that are not easily available from the Java world. Things like the Android SDK, the PhantomJS driver binary, the chromedriver server binary, containers downloaded via complicated Maven commands and similar blobs. 
...
* should be externally configurable (via arquillian.xml) - selected metadata could be overrided in arquillian.xml
