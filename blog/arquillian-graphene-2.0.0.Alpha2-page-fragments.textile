...
h3. Now, could you please ask yourself the following questions.
...
p. Notice the @@Root@ annotation. The value for that field is automatically injected by Graphene to reference all defined @@FindBy@ elements from it. Such fragment implementation is pretty generic and therefore reusable in all tests for different applications. Finally it would encapsulates all the services which this fragment provides, but it does not have to all. You can for example encapsulate browser specific interactions with this fragment. There are no boundaries!
...
p. Notice integration with Page Object pattern, which is achieved by @@Page@ annotation. As your application will grow, the only what you will need to change will be root references of your Page Fragments. Last but not least you will be able to reuse this Autocomplete Page Fragment in other tests for maybe different applications. Isn't it awesome ?
