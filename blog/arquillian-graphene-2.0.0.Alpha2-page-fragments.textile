...
h3. Consider please following snippet from Arquillian Graphene functional test <br /> for "simple page":http://showcase.richfaces.org/richfaces/component-sample.jsf?demo=autocomplete&sample=modes&skin=blueSky, which contains three Autocomplete widgets.
...
bc(prettify).. public class TestAutocompleteWidgets extends AbstractGrapheneTest {
    ...
    public void testFirstAutocomplete() {
        ...
        assertFalse(graphene.isElementPresent(selection),
            "The selection of suggestions should not be visible, since there is only one char!");
        ...
        assertTrue(graphene.isVisible(selection), "The selection of suggestions should be visible, since there are two chars!");
    ...
    public void testSecondAutocomplete() {
        ...
        assertTrue(selenium.isVisible(selection),
            "The selection of suggestions should be visible, since there is correct starting char!");
...
bc(prettify).. public class TestPage {
    ...
    //all the getters methods for fields

    //other handy methods which you find useful when testing those three Autocomplete widgets
}

p. Declaring Page Fragments in the Page Object is one of the options (the preffered one, the other is to declare Page Fragments directly in tests) to go with. *It is done just by using WebDriver @@FindBy@ annotation with reference to the root of the component.* So simple! 
...
public class TestWhichUsesPageFragments extend AbstractTest {
    ...
    public void testFirstAutocomplete {
        ...
        assertFalse(autocomplete.areSuggestionsAvailable(), "The selection of suggestions should not be visible, since there is only one char!");

        String keys = "ar";
        autocomplete.type(keys);

        assertTrue(autocomplete.areSuggestionsAvailable(), "The selection of suggestions should be visible, since there are two chars!");
    ...
    public void testSecondAutocomplete() {
        ...
        String errorMsg = "The selection of suggestions should be visible, since there was typed correct char ";
...
p. Notice integration with Page Object pattern, which is achieved by @@Page@ annotation. As your application will grow, the only what you will need to change will be root references of your Page Fragments. Isn't it awesome ?
