...
"Request guards":https://docs.jboss.org/author/display/ARQGRA2/Request+Guards block the Selenium test execution until a network communication caused by a given action ends. Guards support blocking on _HTTP_ and _XHR (AJAX)_ but can also be used to verify that no such request were done.
