...
!(graphic pull-right)/images/blog/drone_400.png! We've please to announce Drone 1.2.0.Alpha3. This release brings you PhantomJS binary auto resolution and a major update of SPI for extension creators.
...
PhantomJS binary is now automatically resolved if not found using @phantomjs.binary.path@ property. So you don't have to specify this property anymore.
...
If you are writing your own Arquillian extension and you would like to use Arquillian Drone, it is now much simpler. Drone exposes events for browser configuration, creation, enrichement, enhancement and disposal, giving you full control of its lifecycle. If you have used old SPI model, there are no other interfaces you might need to implement. 
=:

- <i class="icon-star"></i> Updated to WebDriver 2.33.0 :=
Arquillian Drone uses WebDriver 2.33.0 by by default. You can still use @selenium-bom@ to override WebDriver versions if needed. 
=:
