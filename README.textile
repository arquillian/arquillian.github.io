...
Now you are ready to install awestruct. The remainder of the commands work on any Ruby installation. If you're using JRuby, remember to use @jgem@ instead of @gem@.
...
*NOTE* If you're using JRuby, you'll need to update your RedCloth gem to support international characters. Follow these steps:

* Download https://github.com/downloads/mojavelinux/redcloth/redcloth_scan.jar
* Copy it to $JRUBY_HOME/lib/ruby/gems/1.8/gems/RedCloth-4.2.5-java/lib/ (overwrite existing file)
...
Install these Gems using the @gem install@ command (or @jgem install@ command for JRuby) as you did above.
