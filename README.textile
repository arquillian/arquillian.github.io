...
h3. Required Compass Fixes

The Arquillian website generator uses the CSS authoring framework "Compass":http://compass-style.org. The website source uses Compass 0.11.5, but currently Awestruct (0.2.3) only allows Compass <= 0.10.6. Until the Compass dependency in Awestruct is updated, you need to "hack" your local Awestruct install to allow version 0.11.5 to be loaded.

Open up the Awestruct gemspec:

bc. $GEM_HOME/specifications/awestruct-0.2.3.gemspec

Replace all occurances of @<= 0.10.6@ with @>= 0.11.5@, then save the file. (You may need to be root).

You also need to fix the configuration of the Compass images directory that is set incorrectly by Awestruct.

Open up the Awestruct engine.rb file:

bc. $GEM_HOME/gems/awestruct-0.2.3/lib/awestruct/engine.rb

Replace line that sets the variable Compass.configuration.images_dir as shown below, then save the file. (You may need to be root).

bc. Compass.configuration.images_dir = 'images'

You are now ready to generate the site.
...
* extract guide h2.header styling into extension (also include authors & title)
...
* link to FAQ? (other than in guides index)
...
* add share buttons in footer (facebook, google+, etc)
* use news icon in place of RSS icon for subscribing to blog
...
* header section on home page so that page can be white
* rename guides #section div to something more appropriate (perhaps use #content-header)
* make slide.js responsive to resize and zoom (currently hacked up)
...
** should be set to Compass.configuration.images_dir = 'images'
...
* don't try to generate page ending in ~
