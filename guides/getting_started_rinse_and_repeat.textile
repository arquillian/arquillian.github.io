...
By default in JUnit, @@Test@ methods are executed in random order. Arquillian's JUnit integration provides an @@InSequence@ annotation that can be used to define an explicit execution order. We'll use @@InSequence@ in this test case to ensure the repository contains the items in the order placed by the first test method.
...
In your Maven pom.xml file, add two new profiles under the @<profiles>@ element. The first profile uses a remote JBoss AS 7.1 container:
...
Recall that there are two ways to activate a Maven profile in Eclipse when using the "Maven Integration for Eclipse":http://www.eclipse.org/m2e:
