...
description: Μάθε πώς να χρησιμοποιείς το JBoss Forge ώστε να εξοικειωθείς ταχύτερα με το Arquillian και να εργάζεσαι πιο αποτελεσματικά καθώς προγραμματίζεις τα τέστ.
guide_group: 1
guide_order: 30
---
...
* Δημιουργήσεις ένα component και το αντίστοιχο Arquillian τέστ
* Εκτελέσεις ένα Arquillian τέστ σε πολλούς containers χωρίς χειροκίνητες ρυθμίσεις

Πολλά από τα βήματα που θα εκτελέσεις περιέχονται στον "Getting Started":/guides/getting_started/ οδηγό, με τη διαφορά ότι το Forge αναλαμβάνει την "βρώμικη" δουλειά για εσένα. Σχεδιάσαμε αυτόν τον οδηγό ώστε να διαβάζεται γρήγορα και να ξεκινήσεις γρηγορότερα από ποτέ!

h3. Παραδοχές

Ο οδηγός αυτός προυποθέτει ότι έχεις εγκαταστήσει το "JBoss Forge":http://jboss.org/forge. Η εγκατάσταση του Forge είναι μια σχετικά σύντομη διαδικασία. Ανάτρεξε στον οδηγό "Getting Started":/guides/getting_started/ ή στο "official Forge documentation":https://docs.jboss.org/author/display/FORGE/Home για οδηγίες λήψης και εγκατάστασης. Επίσης θα χρειαστείς το "JDK(Java Development Kit) 1.6":http://www.oracle.com/technetwork/java/javase/downloads/jdk-6u29-download-513648.html ή μεγαλύτερη έκδοσης εγκατεστημένο στο μηχάνημά σου.
...
Το project έχει τώρα ένα component. Επέστρεψε στον root φακελο του projecτ για να το τεστάρεις.

bc(command). $ cd ~~

h3. Εγκατάσταση Arquillian

Η εγκατάσταση του Arquillian είναι εύκολη όπως ίσως θα έχεις δει στον οδηγό "Getting Started":/guides/getting_started/. Ωστόσο, χρειάζεται αρκετό copy/paste για το σετάρισμα ενός Arquillian container adapter στο pom.xml. Το Forge μπορεί να το κάνει αυτό για εσένα με μια εντολή!
...
Το plugin είναι έτοιμο για χρήση. Τώρα πρέπει να σετάρουμε το Arquillian και έναν container adapter. Θα ρυθμίσουμε το Arquillian για χρήση με τον JBoss AS 7.

bc(command). $ arquillian setup --container JBOSS_AS_MANAGED_7.X

Το Forge θα σου ζητήσει να ορίσεις ποιά έκδοση του JUnit, Arquillian και JBoss AS 7 να χρησιμοποιήσει. Χρησιμοποίησε τις τελευταίες εκδόσεις για καθένα από τα παραπάνω.
...
- Αν απαντήσεις @y@: := Το Forge θα σε ρωτήσει πού θέλεις να εγκαταστήσεις τον JBoss AS 7 (e.g., /home/ike/applications). Στη συνέχεια το Forge ρυθμίζει το Maven dependency plugin ώστε να κατεβάσει και να αποσυμπιέσει τα περιεχόμενα του AS 7 σε αυτή την τοποθεσία (αν δεν βρίσκονται ήδη εκεί) πριν τα tests εκτελεστούν.
- Αν απαντήσεις @N@: := Θα χρειαστείς μια υπάρχουσα εγκατάσταση του JBoss AS 7 στο μηχάνημά σου.

Σε κάθε περίπτωση, πρέπει να γνωρίζει το Arquillian που βρίσκονται τα αρχεία εγκατάστασης του JBoss AS 7. Ένας τρόπος να γίνει αυτό είναι να ορίσεις την JBOSS_HOME environment μεταβλητή ώστε να δείχνει στο φάκελο εγκατάστασης του AS 7. Ο προτιμότερος τρόπος ειναι να σετάρεις το installation path ρητά χρησιμοποιώντας το @jbossHome@ container configuration property στο arquillian.xml αρχείο. Το Forge μπορεί να βοηθήσει με αυτό ως εξής:
...
p. Όπως βλεπεις, το test ενσωματώνει την class που θα τεσταριστεί  (@MySimpleBean@) χρησιμποιώντας CDI, επομένως αυτό αποδεικνύει αν το test εκτελείται μέσα στον container.
...
Πριν ξεκινήσουμε να γράφουμε tests που χρησιμοποιούν το Java Persistence API (JPA), πρέπει να ρυθμίσουμε το JPA. Στη συνέχεια θα δημιουργήσουμε ένα Entity και το αντίστοιχο Data Access Object. Αυτές είναι οι classes που θα τεστάρουμε.
...
Τώρα χρειάζεται να προσθέσουμε ένα Data Access Object (DAO) να διαχειρίζεται αυτήν την οντότητα/entity. Αντέγραψε την παρακάτω dao class μέσα στο project.
...
Τώρα φτιάξε μια νέα test μέθοδο που κάνει επιβεβαιώνει την ορθή λειτουργία της @listLanguages()@.
...
p(warning). %Μερικά πρόσθετα βήματα απαιτούνται για να τρέξει αυτό το τέστ σε ένα GlassFish instance. Το αφήνουμε για άσκηση ~:)%
...
Αν ανοίξεις το jar αρχείο χρησιμοπιώντας ένα archive πρόγραμμα, θα δεις ότι ταιριάζει με το ShrinkWrap archive που όρισες στην @@Deployment@ μέθοδο του τέστ.
