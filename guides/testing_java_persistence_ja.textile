...
description: データをテストしよう！Arquillianテストで、複数のプロバイダに対して、どのようにJava Persistence (JPA) クエリをテストするか学びます。
guide_group: 2
guide_order: 1
TODO: ["use utility EJB to handle transaction boundaries?"]
---
このガイドでは、Java Persistence (JPA) データレイヤのテストにArquillianをどのように使うか、説明します。このガイドを読むと、以下のことができるようになります：
