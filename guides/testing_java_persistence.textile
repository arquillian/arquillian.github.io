...
p(warn). %You won't be able to use the arquillian-weld-ee-embedded profile with this project since Weld does not provide the JPA service (it only provides CDI).%
...
p(warn). %Arquillian doesn't yet support deploying a DataSource to JBoss AS--though it's in the pipeline. For now, we'll use the built-in DataSource in JBoss AS 7, java:jboss/datasources/ExampleDS.%
...
p(warn). %Make sure your JBOSS_HOME environment variable points to a JBoss AS 7.1.1.Final installation. Alternatively, you can set the location using the jbossHome property in arquillian.xml.%
