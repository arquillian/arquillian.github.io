...
Deze handleiding gaat er van uit dat maven beschikbaar is op de opdrachtprompt of in je IDE(Integrated Development Environment). Zo niet, "installeer Maven nu":maven_download. Je moet ook "JDK(Java Development Kit) 1.6":jdk_download of beter geïnstalleerd hebben.
...
Allereerst, creëer een Maven gebaseerd Java project met het volgende commando. _Kopieer de tekst na het @$@ teken en plak het in je commando regel._

bc(command).. $ mvn archetype:generate -DarchetypeGroupId=net.avh4.mvn.archetype \
-DarchetypeArtifactId=java-1.6-archetype

p. Voeg, wanneer je gevraagd wordt de volgende waardes achter de dubbele punt in en bevestig na iedere regel met @<ENTER>@.
...
p(info). %Het project is ingesteld om Java6 en Junit 4.8, de minimale versies van Java en Junit om te testen met Arquillian te gebruiken.%
...
p. Je hebt je eerste Arquillian test geschreven! Ah, je denkt vast hoe moet ik dit uitvoeren ~:S Als je denkt, "Gewoon als een unit test" dan heb je het goed! Maar, eerst moeten we nog een container adapter toevoegen aan het classpath.
...
Je kunt ook maven dit werk voor je laten doen, door het volgende XML fragment onder het @<id>@ element van @arq-jbossas-managed@ profiel toe te voegen:
...
p. Om dit te gebruiken moet je Arquillian een beetje configureren. Creëer de volgende file en laat @jbossHome@ wijzen naar de locatie waar JBoss AS 7 geïnstalleerd is. Als je de Maven plugin gebuikt dan is de locatie @target/jboss-as-7.0.2.Final@.
...
Dat was _dezelfde_ test, maar dit keer in een standaard (niet embedded) Java EE container. Arquillian zorg voor het samen pakken van de test het deployen binnen de container als Java EE archief, het uitvoeren van de test en de resultaten samenvatten en binnen Eclipse JUnit view. (of binnen Maven surefire). Je kunt meer lezen over hoe dit werk in de 
"Arquillian reference guide":https://docs.jboss.org/author/display/ARQ/Negotiating+test+execution.
