...
本教程假定你已经在使用 Maven，不管是通过命令行，还是 IDE(Integrated Development Environment)。如果还没使用，请参考 "现在就安装 Maven":maven_download 。另外，你的机器必须安装 "JDK(Java Development Kit) 1.6":jdk_download 或以上版本。
...
p(info). %Java EE API 依赖已经移到这个 profile 中，因为一些容器，比如 Embedded GlassFish，已经提供这些库文件，同时包含在 classpath 中会引起冲突，所以不得不在 classpath 上花心思。%
