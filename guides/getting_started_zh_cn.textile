...
到目前为止，使用 "JBoss Forge":forge_home 是比较简单的方法，但是本教程依然提供两个选项，考虑到你可以还没有采用 JBoss Forge。可以选择上面两者之一直接跳到相应的介绍环节。
...
p(info). % 项目已经预先配置了使用 Java 6 和 JUnit 4.8 ，这也是使用 Arquillian 必需的 Java 和 JUnit 最小版本。%

生成工具还同时在两个 @java@ 文件夹下创建一个名为 @org.arquillian.example@ 的 Java 包。你应该把你的 Java 源文件放到这个包中，而不是 @java@ 文件夹的根目录下。

p(warning). %Arquillian 同样支持 TestNG 5. 但是，本教程中 JUnit 。 %
...
安装 Forge is 不需要太多时间，本教程会教你从基础学起。按照下面的步骤来安装：
...
p(info). % Windows 系统上，你可以用鼠标右击打开 "Control Panel"，然后点击 "System Properties"，打开 "Advanced" 标签，然后打开 "Environment Variables" ，添加上面两条信息。建议设置到用户变量中，除非你已经所有用户都可以访问解压的目录。%
...
要写一个 Arquillian 测试，我还需要一个用于测试的组件。我们先从创建一个最基本的组件开始，来学习如何运行 Arquillian 测试。后来，我们再探讨一些更复杂的场景。

在 IDE，在 @org.arquillian.example@  创建一个名为 @Greeter@ 的 Java 类。将文件内容替换成如下：
...
一旦将所有必须的 Arquillian 库添加到  classpath，你就可以像运行单元测试那样来运行 Arquillian 测试，不管你是从 IDE 中运行，还是构建脚本或者是其他测试插件。下面我们从 Eclipse 中运行测试。test just like a unit test, whether you are running it from the IDE, the build script or any other test plugin. Let's run the test in Eclipse.
...
然后你应该会看到 JUnit View 弹出来了，显示 *(greenbar)green bar*!
...
这里不打算长篇大论。为什么呢，因为 Arquillian 测试是如此直接，就如你调试一个单元测试一样。仅仅是在任何地方（测试代码或者是程序代码）添加一个断点。然后右键点击文件，选择 Debug As > JUnit Test 。你现在是在一个容器中进行调试。 You're now debugging in the container! Have fun poking around!
...
这是 _同一_ 测试，这次是在标准 (非嵌入式) Java EE 容器中运行。 Arquillian 负责打包，以 Java EE 归档形式部署到容器中，远程的执行测试，收集执行结果，反馈给 Eclipse JUnit 结果视图 (或者写入 Maven surefire 测试报告中)。 更多有关生命周期的细节，请参考 "Arquillian 参考手册":https://docs.jboss.org/author/display/ARQ/Negotiating+test+execution.
