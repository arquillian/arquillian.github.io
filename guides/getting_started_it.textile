...
p(info). %Per default, JBoss AS 7 esegue il deploy degli archivi di test tramite il protocollo JMX. Noi siamo andati oltre e abbiamo aggiunto un protocollo Servlet nel profilo @arquillian-jbossas-managed@ nel caso possa essere utile in futuro. Vedete "queste FAQ":https://community.jboss.org/wiki/WhyDoIGetNoActiveContextsForScopeTypeWhenTestingOnJBossAS7 per le istruzioni di utilizzo.%

p. Ora possiamo scegliere di lanciare il test su tre container differenti.

p. Se avete problemi con il pom.xml a questo punto, potete scaricare il file "pom.xml":https://raw.github.com/arquillian/arquillian-examples/master/arquillian-tutorial/pom.xml dal "progetto d'esempio":https://github.com/arquillian/arquillian-examples/tree/master/arquillian-tutorial.

h3. Test Attraverso i Containers

Riaggiornando il progetto da Eclipse noterete che non compila più. Questo perchè avrete bisogno di attivare uno dei profili dei container. Attiviamo il Weld EE embedded per ritornare alla situazione precedente.

Ci sono due modi per attivare un profilo Maven in Eclipse (presupponendo che stiate usando il "Maven Integration for Eclipse":http://www.eclipse.org/m2e):
...
Per attivare manualmente il profilo, seguite i seguenti passaggi: 
...
Quì in basso una finestra delle proprietà di Maven che mostra il profilo attivato:
...
Quì in basso la finesra del selettore del profilo che mostra il profilo attivato:
...
Abbiamo già visto che il test funziona con il Weld EE Embedded. Eseguiamo una nuova prova facendo uno switch al profilo del GlassFish integrato ripetendo i passaggi visti sopra, questa volta attivando il profilo @arquillian-glassfish-embedded@.
...
Abbiamo eseguito lo stesso test su due differenti container integrati, un container CDI e un container Java EE (GlassFish). Entrambe le esecuzioni avvengono in un solo processo. Per assicurarci che il componente lavori su un ambiente più veritiero, abbiamo bisogno di usare un container standalone. Eseguiamo lo switch a JBoss AS.
...
Una volta installato JBoss AS (per la precisione, estratto) e la vostra variabile d'ambiente @JBOSS_HOME@ è configurata nel path corretto, potete attivare il profilo @arquillian-jbossas-managed@ e lanciare il test. Dovreste vedere l'avvio di JBoss AS nella console...e un'altra *(greenbar)barra verde*!
...
p. Ora cambiamo il profilo Maven puntandolo a @arquillian-jbossas-managed@, e rilanciamo il test. Dovreste vedere l'avvio di JBoss AS dalla console...e ancora la *(greenbar)barra verde*!
...
Per uno sguardo più approfondito su Arquillian, potete passare alla guida "Guida Introduttiva: Rispolveriamo e Ripetiamo":/guides/getting_started_rinse_and_repeat. Per vedere come Forge automatizza la configurazione di Arquillian e genera i test, passate alla guida "Rapida Guida Introduttiva a Forge":/guides/get_started_faster_with_forge.
