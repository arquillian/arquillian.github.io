...
The last bit of information which misses is to show you @pom.xml@ file. This one will be pretty concise too. For the sake of simplicity, we as showing only one profile which is activated by default. We will be testing in managed container of JBoss AS 7.
...
Testing is very easy since the profile is activated by default. The only thing which remains is to run test
