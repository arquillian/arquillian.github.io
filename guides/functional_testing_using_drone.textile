...
guide_summary: Discover how Arquillian Drone simplifies the use of Selenium 2 to test the web UI of your application.
guide_group: 2
guide_order: 3
---
...
p(important). %If get java.lang.ClassNotFoundException: com.thoughtworks.selenium.DefaultSelenium later when you run the test, likely you don't have the testable = false attribute set on the @@Deployment@ annotation. Don't forget it!%

p(info). %Keep in mind that ShrinkWrap can add a package, directory and so on, so your method can be made far more concise.%
...
p(info). %The @isElementPresent()@ method in the DefaultSelenium API accepts an XPath expression as a string. Don't confuse it with the similarly-named method from the Ajocado API that accepts an XPathLocator object.%
