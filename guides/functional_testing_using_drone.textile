...
Arquillian Graphene is a set of extensions for "WebDriver API":http://docs.seleniumhq.org, focused on rapid development and usability in Java environment. Its API encourages people to write tests for AJAX-based web applications in consise and maintainable way. Graphene strives for reusable tests by simplifying the use of web page abstractions ("Page Objects":https://docs.jboss.org/author/display/ARQGRA2/Page+Objects and "Page Fragments":https://docs.jboss.org/author/display/ARQGRA2/Page+Fragments). You will get taste of Graphene API just in a minute!

Arquillian Graphene depends on Arquillian Drone, which is an extension that manages the life cycle of tested browsers. Drone simplifies the initial test setup and prepares tests for continous integration in variety of web browsers.
 
Currently, the list of supported browsers is pretty large, a bounded set of those covered by Selenium project, e.g. Chrome, Firefox, Interner Explorer, Safari, Opera. It also supports "headless browsers":http://blog.arhg.net/2009/10/what-is-headless-browser.html PhantomJS and HTMLUnit. Like Arquillian Core, the Arquillian Graphene and  Arquillian Drone extensions are, well, pretty extensible.
...
p. You have to specify the dependency on a container adapter. We will use JBoss AS 7 managed container since it boots extremely fast!
...
p(info). %"PhantomJS":http://phantomjs.org/ can be used as default browser thanks to its ability to operate in resource-limited environments such as continous integration servers while closely simulating behavior of Chrome browser.%
...
p. Then we can finally start to control the page to test its behavior.
...
p. In *step 2*, we have injected a handle for element using @FindBy@ annotation. This annotation is known from being base for page logic encapsulation.

p. In *step 3*, we are just controlling the page using @WebElement@ API - the entry point for operations with DOM elements.

p(info). %Do you like how HTML and CSS helps you to split logic and presentation? In the same sense, @FindBy@ annotation splits page structure and test logic. You will get know more advanced usage of page abstraction later in this guide.%
...
In order to test your page without executing any real browsers, let's disable all profiles. Your test will run against headless PhantomJS browser you have specified as default earlier.%

*Oh, wait!* But we haven't tested anything yet, we have just opened the page and written text into text inputs.


h3. Defining the Test Logic

Let's define what we will actually test: when user logs into into the application, he can use an AJAX widget which prints his name.
...
* @@FindByJQuery("selector")@ - jQuery selectors is CSS selector syntax with powerful extensions
...
p(info). %As you will experience, most of the actions on the AJAX-enabled pages doesn't block the Selenium execution. In the end, you will need to synchronize almost the page interactions. In fact, in order to be sure the tests weren't fail intermitently no matter where they run, you should synchronize each manipulation with the page. Only that way you will ensure reproducibility.%

p(important). %You should forget about using delays as a method of synchronization in your tests. The poorely written test can run on your development machine but it can simply fail in more problematic environments.%
...
p. *That's it!* It is necessary just wrap two page interactions!

p. When we click on the login button (*step 1*), we are re-locating to another page, so we have used "Request Guards":https://docs.jboss.org/author/display/ARQGRA2/Request+Guards to wait until the full-page update ends.

p. Then we click on a button which discovers user's name and we guard AJAX request - the test won't continue until the page is fully updated as a consequence of an AJAX request.
...
The situations where the waiting API or implicit waiting won't help you:
...
p. Let's use the Request Guards where you can as they are simplest synchronization API and use Waiting API everywhere else.

h3. Abstract the Pages and their Fragments

p. As you would continue to author new tests as application grows, it will quickly become apparent that you need to tame a size of growing functional test suite.
...
* as each action in application needs to be authentizated, tests will need to go through login action repeatedly
...
p. Since we have abstracted the pages into own classes, we are now able to thoroughtly simplify the test:
...
p. Run the tests and againt... *(greenbar)Green Bar*

p. Awesome! Page Objects pattern has been applied.

p. Graphene initializes the page objects wherever it see @Page@ annotation. In a special case it injects an @InitialPage@ which looks into the page object class for @Location@ annotation.

p(info). %What is the added value of this refactoring? Tests are decoupled from the HTML code, so they are more readable. Introduction of any a change to the application would be less error prone as tests are more robust.%
...
p(info). %During the test development, we have used a managed container. This really isn't the fastest way how to develop functional tests, but the goal of this guide is just teach you the basics. You can refer to tips & tricks how to achieve "rapid test development turnaround":http://lukas.fryc.eu/blog/2013/06/testing-jsf-richfaces-arquillian-2-rapid-test-development.html.%

p(info). %Even though this guide follows best practices to manage Graphene guide including few tricks for allowing continuous integration, you should really "get know more if you thrives to achieve good coverage":http://lukas.fryc.eu/blog/2013/06/testing-jsf-richfaces-arquillian-4-continous-integration.html and guard your test against regressions across multiple browsers.%
