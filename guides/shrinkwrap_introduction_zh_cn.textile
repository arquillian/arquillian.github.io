...
在 Maven 中， 利用传递性依赖 POM， 这些可以放在不同的 Scope 中，从 Maven Central 资源库获取：
...
p. 为了照顾没有使用 Maven 资源库系统的项目，ShrinkWrap 发布包将所有的模块放在一个下载文件中，你可以根据自己的需要手动设置依赖。
...
p. 现在看看我们做了些什么。首先，我们创建了一个  @JavaArchive@ ，加入当前的 @Class@ 所在 @Package@ 的所有内容。然后我们打印输出到控制台，只是为了看看包含了哪些内容。最后一行，我们再次利用 @JavaArchive@ 中来自 @Assignable@ 的工具切换到一种新的视图：输出 ZIP 格式。本例中，我们使用相应的  @ZipExporter@ ，允许输出到一个 @File@, @OutputStream@, 甚至将内容转化为 @InputStream@ ，这样我们还可以进行字节码操作。
