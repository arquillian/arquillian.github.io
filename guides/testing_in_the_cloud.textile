---
layout: guide
title: Testing in the Cloud with Arquillian
authors: ['Marek Schmidt']
tags: [openshift, as7]
guide_summary: Test your application on OpenShift.
guide_group: 2
guide_order: 4
TODO: ["OpenShift introduction"]
---
This guide introduces you to the Arquillian OpenShift Container. After reading this guide, you'll be able to:

* Create an account on a free PaaS, OpenShift Express.
* Configure Arquillian OpenShift Container

h3. Assumptions

We'll assume that you've read the "Getting Started":/guides/getting_started guide and the "Functional Testing":/guides/functional_testing_using_drone 

This tutorial builds upon the application that you have built in the previous tutorial.

We'll be running the tests on an AS7 cartrige on OpenShift and we will show you how to get a free OpenShift account.
...
You can now sign-in into OpenShift with your credentials. We will be using OpenShift Express, so click on "Express". OpenShift Express is usually managed with client tools, but their use in this tutorial is optional as we can create an app from the OpenShift Express Web Console. See TODO for more documentation and tutorials about using OpenShift.
...
You can now try to go to the URL of the app in your web browser, you should see a page with the message "Welcome to OpenShift, JBossAS7 Cartridge". You can now skip the next part and go to TODO.
...
h3. Create an OpenShift Maven Profile

p. Add another entry to the dependency managemenet section defining version of the Arquillian OpenShift Express container.
...
p. Next create a new arquillian-openshift-express profile.
...
You can now test in the cloud with the following command:
