...
最初，ShrinkWrap 的诞生是出于更简单了测试 Java 企业部署的需要。传统的基于普通文件归档源于 ZIP 标准，这样必需引入一些构建步骤，对所有应用资源进行打包。 一个构建步骤要花费不少时间：
...
所以我们会问： “如果在 Java 中我们能够声明一个对象代表归档，结果会有什么不同？"

结果是一个 Java API 模拟 "jar" 工具，一个具有直观语法的虚拟文件系统。
...
p. 结果是利用 IDE 增量编译的特性，允许我们路过构建过程。
...
第一步是让你了解 ShrinkWrap 二进制文件结构，核心由三部分组成：
...
只有 API 是必须在编译的 Classpath 中， SPI 和 Implementation 在运行时需要。这样很好的将直接使用和项目内部类进行了分离。

在 Maven 中， 利用传递性依赖 POM， 这些可以放在不同的Scope中， 可以从 Maven Central 资源库获取：
...
p. 为了照顾没有使用 Maven 资源库系统的项目，ShrinkWrap 发布包将所有的模块放一相下载文件，你可以根据自己的需要手动设置依赖。
...
另外， @Archive@ 包括多个视图，通常情况下，你不会直接处理 @Archive@ 类。相反，ShrinkWrap 提供了一些 @Archive@ 扩展，自带了一些有用的方法来操作类型相关的内容。
...
要创建一个 @Archive@, 只要简单的选择你要创建的归档类型，根据需要，可以添加一个名字到静态 @ShrinkWrap:create@ 方法中：
...
当然一个代表空归档的对象是毫无用处。所以我们试着看添加一些内容。正如前面所说的那样，内容由 @Asset@ 进行构造，所以我们先看 ShrinkWrap 提供的一些 @Asset@ 实现：
...
另外，因为 @Asset@ 是一个接口，你可以提供你自己的实现，来处理任何代表 @InputStream@ 基于字节的内容。例如，下面的代码片断演示如何将一个 Activation Framework @DataSource@ 转换成 @Asset@ :
...
p. 传递一个 @true@ 标志给  @Archive@ 的 @toString@ 方法来打印递归的 @"ls -l"@ 样式的输出:
...
p.  根据你要操作的不同内容的类型， 我们先前提级的 @Archive@ 视图非常有用。例如，一个标准的 JAR 文件一般包含 @.class@ 文件和其它资源，所以你可以用 @JavaArchive@ 类型来添加这些内容。  

ShrinkWrap 支持一种简单的机制，允许你的归档文件在不同的视图间切换， 由  @org.jboss.shrinkwrap.api.Assignable@ 接口的  @as@ 方法提供，每种视图都是从  @Assignable@ 继承。所以要让你的归档用上 @JavaArchive@ 视图，要添加 @Class@ 资源， 可以简化为:
...
p. 使用这种机制的好处是让保持 ShrinkWrap 使用简洁和直观，同时提供多重继承语言的灵活性。

h3. 操作文件内容

因为 ShrinkWrap 扎根于 Java EE 并与 Arquillian 测试平台紧密联系，但不局限这些领域。事实上，ShrinkWrap 更偏向扮演一个归档文件的虚拟系统的角色。因此，它提供了一种简单机制来操作普通文件结构。
...
p. 现在看看我们做了些什么。首先，我们创建了一个  @JavaArchive@， 加入当前的 @Class@ 所在 @Package@ 的所有内容。然后我们打印输出到控制台，只是为了看看包含了哪些内容。最后一行，我们再次利用 @JavaArchive@ 中来自 @Assignable@ 的工具切换到一种新的视图：输出 ZIP 格式。本例中，我们使用合适的  @ZipExporter@， 允许我们输出到一个 @File@, @OutputStream@, 甚至将内容转化为 @InputStream@， 这样我们还可以进行字节操作。
...
当然，我们也可以依葫芦画瓢使用下面标准的 importers 之一从普通文件来获得一个 ShrinkWrap 归档：
...
p. 注意我们是如何 @ZipImporter@ 给 @ShrinkWrap.create@ 方法的，它也是一个 @Assignable@!  开始注意到这个了吗?

这里我们简短的介绍了如何 ShrinkWrap 操作归档文件内容。 我们希望你能够进行研究 API，它更直观，更一致，欢迎到我们社区。