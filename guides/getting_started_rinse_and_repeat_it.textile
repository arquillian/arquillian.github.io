...
Vedrete le seguenti righe stampate nella console:
...
*Congratulazioni!* E' stata eseguita la vostra prima  *(greenbar)barra verde* con Arquillian usando un container remoto.
...
Abbiamo terminato la prova con JBoss AS, ora clicchiamo su sulla finestra Servers per fermare il server. Eseguiamo ora il test su GlassFish 3.1.2.

h4. Lanciare il Test su GlassFish

Per lanciare il test sul server standalone GlassFish, lanciamo per prima cosa il server. Apriamo la finestra Servers, selezioniamo GlassFish 3.1.2, r clicchiamo sul bottone verde play. Attendiamo la fine dello start.
...
Naturalmente, il debug di un test in un container integrato è semplice perchè gira all'interno dello stesso processo del test. Ma come fare con container remoto?
...
Nella finestra Servers, noterete che c'è un'icona di un insetto subito accanto al bottone verde play. Clicchiamo sul bottone per lanciare il server in modalità debug. Eclipse automaticamente si connettterà il suo debugger al container.
...
Se usate il browser dello stack trace nella finestra di Debug di Eclipse, noterete che il server è controllato con un protocollo remoto (JMX o Servlet) e che JUnit è stato lanciato dentro il container.
...
p(info). %Questa configurazione non è necessaria se usiamo un progetto Maven e i sorgenti della libreria sono disponibili in un repository Maven. Quando arrivate ad un breakpoint su di una libreria non ancora disponibile localmente, attendete un pò di tempo per il download e il codice sorgente apparirà. Se non appare, probabilmente il sorgente non è nel repository e ci sarà bisogno di aggiungerlo a mano .%

Quì i passaggi necessari per aggiungere il sorgente della libreria nella configurazione del debug:
...
Dovranno essere eseguiti questi passaggi per ogni classe di test che vogliamo usare per il debug, sebbene nel nostro caso lo facciamo solo una volta (la configurazione del debug va raffinata pezzo per pezzo).
...
Dal momento che Arquillian interagisce sullo start e stop del container, dobbiamo istruire Arquillian a lanciare il container in modlità debug. Si tratta di configurazioni della JVM che Arquillian dovrà passare al server, che noi faremo tramite le proprietà di configurazione del container.

Prima di fare ciò, dobbiamo pensare a come connetterci al debugger. A meno che siate superveloci a usare il mouse, è una vera impresa connettersi al debugger prima che Arquillian inizia il lancio dei test. Fortunatamente, possiamo dire alla JVM di attendere il connettore del debug prima di eseguire l'applicazione, nel nostro caso JBoss AS. Questo ci darà il tempo di cliccare sul bottone debug dell'IDE per connetterci al processo.

Quì c'è la configurazione che dobbiamo aggiungere al descrittore di Arquillian, arquillian.xml, per far si che Arquillian lanci il server in modalità debug: (Assicuratevi che non ci siano spazi subito dopo i tag @<property>@)
...
p. Da notare il flag @suspend=y@. Dice al processo di attendere che il debugger si connetta. Abbiamo configurato la porta su 8787, che dovremo poi mettere nella configurazione del debugger. Ecco i passaggi per lanciare il test:

# Attiviamo il profilo Maven @arquillian-jbossas-managed@ (riferitevi alla "Guida Introduttiva":/guides/getting_started guide)
# Lanciamo il test

Nella Console, dovreste vedere che Arquillian ha lanciato un'istanza di JBoss AS e che sta in attesa che il debugger si connetta:

bc(output). Listening for transport dt_socket at address: 8787

Tutto quello che ora dobbiamo fare è connettere il debugger. Quì vediamo come configurare e lanciare il debugger su Eclipse:
...
A questo punto, avete imparato come lanciare e fare il debug su container integrati, remoti e managed. Non c'è altro da vedere per ora. DIvertitevi a scrivere _test reali_ e siatene orgogliosi!
