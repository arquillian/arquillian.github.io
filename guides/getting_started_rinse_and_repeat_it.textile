...
Riferitevi alla "Guida Introduttiva":/guides/getting_started#test_across_containers per istruzioni su come attivare un profilo. Una volta che il profilo è attivato, il progetto dovrebbe compilare con successo.
...
Quì i passi necessari per aggiungere il sorgente della libreria nella configurazione del debug:
...
Quì c'è la configurazione che dobbiamo aggiungere al descrittore di Arquillian, arquillian.xml, per far si che Arquillian lanci il server in modalità debug: (Assicuriamoci che non ci siano spazi subito dopo i tag @<property>@)
