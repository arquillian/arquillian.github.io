...
p(info). %Naturalmente, testeremo soltanto l'implementazione in-memory. Se volete vedere come testare un'applicazione che usi il database potete consultare la guida "Testare la Persistenza Java":/guides/testing_java_persistence guide. Potrete poi tornare in questa guida e modificarne l'implementazione.%
...
*Congratulazioni!* È stata eseguita la nostra prima  *(greenbar)barra verde* con Arquillian usando un container remoto.

p(info). %È possibile ereditare l'EJB @OrderRepository@ usando l'annotazione @@Inject@ oltre che la @@EJB@. Provate!.%
