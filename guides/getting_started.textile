...
<!--
-# TODO:
-# Setup project with Forge (optional)
-# Explain how to debug with JBoss AS managed
-# Tweet badge of honor for completing tutorial
-# versioning?
-->
...
Open up the @pom.xml@ file once again and add the following XML fragment directly inside the @<dependencies>@ element. Here's how the dependencies section should look once you're done:
...
We also recommend installing JBoss Tools (Indigo), also available in the Eclipse Marketplace (search for "jboss tools").
...
Once again open up the @pom.xml@ file, located in the root folder of the project, in your editor. We need to instruct Maven which versions of the artifacts to use. Insert the following XML fragment directly above the @<build>@ element to import the version matrix for Arquillian's transitive dependencies.
...
p(caution). %There can only be one container adapter on the classpath at a given time.%
...
We'll now include two additional profiles in the @pom.xml@, directly below the @<profile>@ element for Weld EE embedded, one for Glassfish embedded:
...
bc(prettify).. <arquillian xmlns="http://jboss.org/schema/arquillian"
    ...
    <container qualifier="jbossas-7-remote" default="true">
        <configuration>
            <property name="jbossHome">/path/to/jbossas</property>
        </configuration>
    </container>
</arquillian>
...
<a class="post_to_twitter" href="http://twitter.com/home/?status=Just read the %23Arquillian getting started guide and got my first green bar! Get yours. http://arquillian.org/guides/getting_started #greenbar"><img src="/images/post_to_twitter.png" alt="Post to Twitter"/></a>

If you're ready to learn more, check out the "other available guides":/guides.
