...
Arquillian selects the target container based on which container adapter is available on the test classpath. A container adapter is that controls and communicates with the container (it's not a container itself). That means we'll be adding more libraries to the project.
