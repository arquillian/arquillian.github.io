...
guide_group: 1
TODO: ["Setup project w/ Forge as option", "Explain how to debug w/ JBoss AS managed", "Explain why not use official Java EE APIs"]
---
<div class="header">
h2. #{page.title}
...
This guide assumes you have Maven available, either in your command shell or your IDE(Integrated Development Environment). If you don't, please "install it now":http://maven.apache.org/download.html. You'll also need a "JDK(Java Development Kit) 1.6":http://www.oracle.com/technetwork/java/javase/downloads/jdk-6u29-download-513648.html or better.
...
By far, "JBoss Forge":http://jboss.org/forge is the simpler approach, but this guide will offer both options in the event you aren't ready to adopt JBoss Forge. Select from one of the two options above to jump to the instructions.
...
pre. Define value for property 'groupId': : org.arquillian.example <ENTER>
...
p(warning). %If you follow the camp that says repositories should not be explicitly defined in your project's pom.xml, then read "these instructions":http://community.jboss.org/wiki/MavenGettingStarted-Users to enable the repository globally in your settings.xml file.%

p. The foundation of your project is now ready! Let's skip to the next section and "open the project in Eclipse":#open_project_in_eclipse so we can start writing some code!
...
# ["Download Forge":https://repository.jboss.org/nexus/service/local/artifact/maven/redirect?r=releases&g=org.jboss.forge&a=forge-distribution&v=1.0.0.Beta3&e=zip] and unzip it into a folder on your harddrive, which we'll call @$FORGE_HOME@
  We'll assume you extracted it to the @forge@ folder in your home directory
...
On Windows, you'll need to right-click on open the "Control Panel", then click "System Properties", open the "Advanced" tab, then click "Environment Variables" and add these two entries visually. We recommended to set User variables for Forge, unless you have placed the unzipped distribution in a folder where all users can access it.

Now that Forge is extracted (i.e., installed), open a command prompt (shell) and run 'forge':

bc(command). $ forge
[no project] ~ $

That's it! You've got Forge installed. Now it's time to create the project.
...
Forge will create the project and make it your current working directory.

bc(command). [arquillian-tutorial] arquillian-tutorial $ 

By default, Forge sets up the project to use Java 1.6 and the JBoss Public Repository, so you don't have to set any of that up.

What we do need to add is the Java EE APIs. That's done using the @project add-dependency@ command below:

bc(command). $ project add-dependency org.jboss.spec:jboss-javaee-6.0:1.0.0.Final:pom:provided

You'll also need to add JUnit 4 as a test-scoped dependency:

bc(command). $ project add-dependency junit:junit:4.8.1:test

The result of the pom.xml that Forge generated is shown below:
...
Begin by launching Eclipse. Now, since this is a Maven project, you need the "Maven Integration for Eclipse":http://eclipse.org/m2e (m2e) plugin installed to open the project. If you don't already have the integration installed, the simplest way to get it is to install "JBoss Tools":http://www.jboss.org/tools. Follow these steps to install it from the Eclipse Marketplace (sort of like the app store for Eclipse).
...
JBoss Tools provides a nice environment for developing Java EE applications, including CDI support. And don't worry, it's not a heavyweight plugin. However, if you just want the Maven integration without the extras that JBoss Tools brings, you can follow these steps instead:
...
p. If you are having trouble with the pom.xml up to this point, you can download the file from "this gist":https://gist.github.com/1263892.
...
p(warning). %If you get the message "Project configuration is out of date with pom.xml" then "Right click on Project > Maven > Update Project Configuration" to resync the project.%
...
pre. 26 [main] INFO org.jboss.weld.Version - WELD-000900 1.1.1 (Final)
Hello, Earthling!
...
p(important). %If you are using a remote container, Debug As does not cause breakpoints to be activated. You need to start the container in debug mode and attach the debugger. That's because the test is run in a different JVM than the original test runner.%
...
p(warning). %There can only be one container adapter on the classpath at a given time.%
...
p(info). %The Java EE API dependency has been moved to the profile since some containers, like GlassFish embedded, already provide these libraries and would cause conflicts.%

We'll now include two additional profiles in the @pom.xml@, directly below the @<profile>@ element for Weld EE embedded, one for Embedded GlassFish:
...
p. and one for JBoss AS Managed:
...
p. If you are having trouble with the pom.xml up to this point, you can download the file from "this gist":https://gist.github.com/1263934.
...
# Tick the box next to the profile you want to activate (e.g., @arquillian-weld-ee-embedded@)
# Click the OK button
...
# "Download JBoss AS 7":http://jboss.org/as7
...
p. You also need a small bit of Arquillian configuration. Create the following configuration file and assign the value of the @jbossHome@ property to the location where JBoss AS 7 is installed. If you are using the Maven dependency plugin, the location is @target/jboss-as-7.0.2.Final@.
