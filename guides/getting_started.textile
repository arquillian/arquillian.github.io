...
This guide assumes you have Maven available, either in your command shell or your IDE(Integrated Development Environment). If you don't, please "download and install Maven now":maven_download. You'll also need "JDK(Java Development Kit) 1.5":jdk_download or better installed on your machine.
...
p(info). %The project is preconfigured to use Java 1.6 and JUnit 4.8, minimum recommended versions of Java and JUnit for using Arquillian.%
...
p. The foundation of your project is now ready! Skip to the next section to "open the project in Eclipse":#open_the_project_in_eclipse so we can start writing some code!
...
If you want Maven to handle this task for you instead (optional), add the following XML fragment under the @<id>@ element of the @arq-jbossas-managed@ profile:
...
If you want to dive deeper into Arquillian, move on to "Getting Started: Rinse and Repeat":/guides/getting_started_rinse_and_repeat guide. To learn how to use Forge to automate setting up Arquillian, read through "Get Started Faster with Forge":/guides/get_started_faster_with_forge.
