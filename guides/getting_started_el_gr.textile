...
(Optional) Συνιστούμε το upgrade της version του Maven Surefire Plugin για λόγους που περιγράφονται στις "Συχνές Ερωτήσεις":https://community.jboss.org/wiki/WhatVersionOfSurefireShouldIUseToRunMyArquillianTestsInAMavenBuild. Μπορείς να ρυθμίσεις την version του Surefire Plugin προσθέτοντας το @<plugin>@ element μέσα στο @<plugins>@ element, ακριβώς κάτω από το Maven Compiler Plugin:
