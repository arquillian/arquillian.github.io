...
p. Forge adds the JBoss Community repository to the pom.xml file, but this repository is not required to use Arquillian. You can easily remove the configuration using this Forge command:
...
p. Next we will create a new bean.
...
p. The plugin is ready to use. Now we need to set up Arquillian and a container adapter. Let's first use JBoss AS 7.
