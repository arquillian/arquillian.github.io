...
- If you answer @y@: := Forge will ask you where you want to install JBoss AS 7 and will configure the Maven dependency plugin to download and unpack it to that location (if it's not already been done) before the tests are run.
...
That's it! Arquillian is configured.

You can take a look at your pom.xml to see the dependencies and profile that is added. You'll also notice that Forge created an empty arquillian.xml configuration file under src/test/resources, if you should need to configure an Arquillian container adapter or extension.
