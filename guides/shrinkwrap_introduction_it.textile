...
Quindi ci chiediamo: "Possiamo dichiarare in Java un oggeto che rappresenti quest'archivio?"

Il risutato è una API Java analoga al tool "jar", un file system virtuale con una sintassi intuitiva.

div(filename). Creare un archivio ShrinkWrap.
...
p. Per i progetti che non usano Maven, ShrinkWrap ha tutti i moduli disponibili per il download, in modo che chiunque si possa installare manualmente le dipendenze.
...
I JavaDoc per tutte le versioni si trovano qui:  "here":http://docs.jboss.org/shrinkwrap .

h4. Codice Open Source

Fork us and get involved with "Development":http://github.com/shrinkwrap/shrinkwrap .
Puoi fare un fork e aiutaci nello sviluppo collegandoti su "Sviluppo":http://github.com/shrinkwrap/shrinkwrap .
...
|Archivio|@org.jboss.shrinkwrap.api.Archive@|Un insieme di risorse, essenzialmente chiamato virtual filesystem|
|File|@org.jboss.shrinkwrap.api.Node@|An entry in an @Archive@; può rappresentare un contenuto o una directory|
|Path|@org.jboss.shrinkwrap.api.ArchivePath@|Spazio dell' @Archive@ sotto cui giace il @Node@|
|Asset|@org.jboss.shrinkwrap.api.Asset@|Contenuto di tipo byte dentro il @Node@|

In aggiunta, L'@Archive@ ha diverse viste, e di solito non si richiama direttamente la classe @Archive@. Infatti, ShrinkWrap supporta diverse estensioni di @Archive@ che offrono un maggior aiuto nella manipolazione del pacchetto.
...
Per creare un @Archive@, semplicemente scegli l'archivio desiderato e opzionalmente assegna un nome al metodo statico @ShrinkWrap:create@:

bc(prettify).. GenericArchive myArchive = ShrinkWrap.create(GenericArchive.class,"myArchive.jar");

p. Questo è tutto!  Hai creato il tuo primo archivio ShrinkWrap!
...
In aggiunta, dato che l'@Asset@ è un'interfaccia, puoi fornire la tua propria implementazione per supportare un contenuto di tipo byte sotto forma di @InputStream@ .  Come esempio, il codice in basso mostra come rappresentare un @DataSource@ per l'Activation Framework sotto forma di @Asset@ :
...
p. Anche le viste @Archive@ mostrate prima sono molto di aiuto, a seconda del tipo di contenuto che si utilizza. Per esempio, uno standard file JAR tipicamente contiene file @.class@ e altre risorse, cosicchè hai modo di aggiungerle tramite il @JavaArchive@.
...
Poichè che ShrinkWrap ha le sue radici nella Java EE ed è legato alla piattaforma di test di Arquillian, non è certamente l'unico uso che se ne può fare. Infatti, ShrinkWrap può lavorare anche con comuni file.
...
Questo conclude la nostra introduzione nel gestire i contenuti con ShrinkWrap. Speriamo che troverete le API molto intuitive e consistenti, e benvenuti alla nostra community.
